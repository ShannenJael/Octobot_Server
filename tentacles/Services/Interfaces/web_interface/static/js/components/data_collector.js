...
function formatDataCollectorPairResult(item) {
    ...
    if (text.indexOf('/') !== -1) {
        ...
        return $(`
            <div class="d-flex justify-content-between align-items-center py-1 px-1">
                ...
                <span class="badge badge-gold-sm ml-2" style="background: rgba(212,175,55,0.25); border: 1px solid #D4AF37; color: #F5D77F; font-size: 10px; padding: 2px 6px; border-radius: 4px;">PAIR</span>
            </div>
        `);
    }
    return item.text;
...
function createSelect2(){
    if ($("#exchangeSelect").hasClass("select2-hidden-accessible")) {
        $("#exchangeSelect").select2('destroy');
    }
    $("#exchangeSelect").select2({
        width: '100%',
        placeholder: "Select an exchange"
    });

    if ($("#symbolsSelect").hasClass("select2-hidden-accessible")) {
        $("#symbolsSelect").select2('destroy');
    }
    $("#symbolsSelect").select2({
        width: '100%',
        closeOnSelect: false,
        placeholder: "Select trading pair(s)",
        templateResult: formatDataCollectorPairResult,
        templateSelection: function(item) {
            return (item.text || "").trim();
        }
    });

    if ($("#timeframesSelect").hasClass("select2-hidden-accessible")) {
        $("#timeframesSelect").select2('destroy');
    }
