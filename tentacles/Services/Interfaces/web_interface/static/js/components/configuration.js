...
function formatTradingPairResult(item) {
    if (!item.id) {
        return item.text;
    }
    ...
    if (text.indexOf('/') !== -1) {
        ...
        return $(`
            <div class="d-flex justify-content-between align-items-center py-1 px-1 select2-pair-result">
                <div class="pair-symbols d-flex align-items-center">
                    <span class="pair-base font-weight-bold text-white">${base}</span>
                    <span class="pair-slash mx-1 font-weight-bold" style="color: #D4AF37;">/</span>
                    <span class="pair-quote font-weight-bold" style="color: #F5D77F;">${quote}</span>
                </div>
                <span class="badge badge-gold-sm ml-2">PAIR</span>
            </div>
        `);
    }
    return item.text;
...
function initCurrencyCarousel(currenciesData) {
    ...
    const $trigger = $("#carouselPickerTrigger");
    const $panel = $("#carouselDropdownPanel");
    const $track = $("#carouselRollingTrack");
    const $viewport = $("#carouselScrollViewport");
    const $searchInput = $("#carouselSearchInput");
    const $searchClear = $("#carouselSearchClear");
    const $counter = $("#carouselMatchCounter");
    const $selectedName = $("#carouselSelectedName");
    const $selectedSymbol = $("#carouselSelectedSymbol");
    const $selectedIcon = $("#carouselSelectedIcon");
    const $arrowIcon = $("#carouselArrowIcon");
    const $nativeSelect = $("#AddCurrencySelect");

    $track.empty();
    const itemsHtml = [];
    const maxItems = 2000;
    const list = currenciesData.slice(0, maxItems);

    let currentSelectedId = $nativeSelect.val() || (list.length ? list[0].i : "");

    list.forEach((item, index) => {
        const isSelected = item.i === currentSelectedId;
        const sym = (item.s || "").toUpperCase();
        const name = item.n || item.i;
        const iconLetter = sym.substring(0, 3) || "COIN";

        itemsHtml.push(`
            <div class="carousel-rolling-item ${isSelected ? 'selected active-highlight' : ''}" 
                 data-currency-id="${item.i}" 
                 data-name="${name.toLowerCase()}" 
                 data-symbol="${sym.toLowerCase()}" 
                 data-index="${index}"
                 role="button" tabindex="0">
                <div class="item-coin-icon-wrapper">
                    <span class="item-coin-avatar">${iconLetter}</span>
                </div>
                <div class="item-details-wrapper flex-grow-1 pl-2 text-left">
                    <div class="item-title-row d-flex align-items-center">
                        <span class="item-coin-name font-weight-bold text-white">${name}</span>
                        <span class="item-coin-symbol badge badge-gold ml-2">${sym}</span>
                    </div>
                    <div class="item-sub-row text-muted small">Cryptocurrency &bull; ${sym} / USDT, BTC, USD</div>
                </div>
                <div class="item-status-wrapper pl-2">
                    <span class="item-selected-badge badge badge-success ${isSelected ? '' : 'd-none'}">
    ...
    $track.html(itemsHtml.join(''));
    $counter.text(`${list.length} cryptocurrencies available`);

    function updateSelection(currencyId, closeDropdown) {
        if (typeof closeDropdown === 'undefined') closeDropdown = true;
        if (!currencyDetailsById[currencyId]) return;
        currentSelectedId = currencyId;
        const details = currencyDetailsById[currencyId];
        const sym = (details.s || "").toUpperCase();
        const name = details.n || details.i;

        // Update native select so backend/AddCurrency logic receives it
        $nativeSelect.val(currencyId).trigger('change');

        // Update trigger preview
        $selectedName.text(name);
        $selectedSymbol.text(sym);
        $selectedIcon.html(`<span class="item-coin-avatar-mini">${sym.substring(0, 3)}</span>`);

        // Update rolling track active highlight
        $track.find('.carousel-rolling-item').removeClass('selected active-highlight');
        $track.find('.item-selected-badge').addClass('d-none');

        const $activeItem = $track.find(`.carousel-rolling-item[data-currency-id="${currencyId}"]`);
        if ($activeItem.length) {
            $activeItem.addClass('selected active-highlight');
            $activeItem.find('.item-selected-badge').removeClass('d-none');
        }
    ...
    function openPanel() {
        $panel.stop(true, true).fadeIn(160);
        $trigger.attr('aria-expanded', 'true').addClass('active');
        $arrowIcon.removeClass('fa-chevron-down').addClass('fa-chevron-up');
        $searchInput.focus();

        // Scroll active item to middle of viewport
        const $activeItem = $track.find('.carousel-rolling-item.selected');
        if ($activeItem.length) {
            const itemOffset = $activeItem.position().top;
            const viewHeight = $viewport.height();
            $viewport.scrollTop($viewport.scrollTop() + itemOffset - (viewHeight / 2) + 25);
        }
    }
    ...
    $(document).off('click.carouselPicker').on('click.carouselPicker', function(e) {
        if (!$(e.target).closest('#currencyCarouselPicker').length) {
            closePanel();
        }
    });

    // Item click
    $track.off('click', '.carousel-rolling-item').on('click', '.carousel-rolling-item', function(e) {
        e.preventDefault();
        const cid = $(this).data('currency-id');
        updateSelection(cid, true);
    });
    ...
    $searchInput.off('input').on('input', function() {
        ...
        let matchCount = 0;
        const $items = $track.find('.carousel-rolling-item');
        ...
        $items.each(function() {
            ...
            const sym = String($(this).attr('data-symbol') || $(this).data('symbol') || "").toLowerCase();
            const cid = String($(this).attr('data-currency-id') || $(this).data('currency-id') || "").toLowerCase();

            if (name.includes(query) || sym.includes(query) || cid.includes(query)) {
                $(this).show();
                matchCount++;
            } else {
                $(this).hide();
        ...
        $counter.text(`Found ${matchCount} match${matchCount === 1 ? '' : 'es'}`);
    });
    ...
    $searchInput.off('keydown').on('keydown', function(e) {
        const $visibleItems = $track.find('.carousel-rolling-item:visible');
        if (!$visibleItems.length) return;

        let $highlighted = $visibleItems.filter('.active-highlight');
        let currentIndex = $visibleItems.index($highlighted);

        if (e.key === 'ArrowDown') {
            e.preventDefault();
            const nextIndex = (currentIndex + 1) % $visibleItems.length;
            const $nextItem = $visibleItems.eq(nextIndex);
            $visibleItems.removeClass('active-highlight');
            $nextItem.addClass('active-highlight');
            $nextItem[0].scrollIntoView({ block: 'nearest', behavior: 'smooth' });
        } else if (e.key === 'ArrowUp') {
            e.preventDefault();
            const prevIndex = (currentIndex - 1 + $visibleItems.length) % $visibleItems.length;
            const $prevItem = $visibleItems.eq(prevIndex);
            $visibleItems.removeClass('active-highlight');
            $prevItem.addClass('active-highlight');
            $prevItem[0].scrollIntoView({ block: 'nearest', behavior: 'smooth' });
        } else if (e.key === 'Enter') {
            e.preventDefault();
            if ($highlighted.length) {
                updateSelection($highlighted.data('currency-id'), true);
            } else if ($visibleItems.length === 1) {
                updateSelection($visibleItems.eq(0).data('currency-id'), true);
            }
    ...
    if (currentSelectedId && currencyDetailsById[currentSelectedId]) {
        updateSelection(currentSelectedId, false);
    }
}
...
function fetch_currencies(){
    ...
    $.get({
        ...
        success: function (data) {
            ...
            // Initialize rolling carousel if present
            if($("#currencyCarouselPicker").length){
                initCurrencyCarousel(data);
            } else {
