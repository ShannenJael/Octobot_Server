...
def register(blueprint):
    ...
    def trading_type_selector():
        ...
        if not enabled_exchanges:
            if config_exchanges:
                enabled_exchanges = [list(config_exchanges.keys())[0]]
            else:
                default_ex = models.get_default_exchange()
                enabled_exchanges = [default_ex]
                if default_ex not in config_exchanges:
                    config_exchanges[default_ex] = {"enabled": True, "sandboxed": True}
